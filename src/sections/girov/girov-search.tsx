...
export function GirovSearch({ query, results, onSearch, loading }: Props) {
  ...
  const handleKeyUp = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (query) {
      if (event.key === 'Enter') {
        const selectItem = results.filter((girov) => girov.name === query)[0];
  ...
  return (
    <Autocomplete
      ...
      renderOption={(props, girov, { inputValue }) => {
        const matches = match(girov.name, inputValue);
        const parts = parse(girov.name, matches);

        return (
          <Box component="li" {...props} onClick={() => handleClick(girov.id)} key={girov.id}>
            <Avatar
              key={girov.id}
              alt={girov.name}
              src={girov.coverUrl}
