...
export const WebCam = (
    { className, onStreamChanged, showSettings, setShowSettings, width, height }:
        WebCamProp
) => {
    ...
    const sendStream = (videoStream: MediaStream, micDeviceId: string) => {
        ...
        navigator.mediaDevices.getUserMedia({ audio: { deviceId: { exact: micDeviceId } } }).then((ms: MediaStream) => {
    ...
    useEffect(() => {
        ...
        let query = webCamConfig.webCamDeviceId != undefined ? {
            video: {
                width: webCamConfig.backgroundOption == 0 ? 1280 : 640, height: webCamConfig.backgroundOption == 0 ? 720 : 480,
    ...
    return (
        <div id='webcam'>
                    ...
                    <div className="modal-body">
                        <div className='webcamSettingsform'>
                            <div className="container">
                                <label htmlFor="webcam"><b>WebCam</b></label>
                                <select id='webcam' value={webCamConfig.webCamDeviceId} onChange={(e) => setWebCamConfig({ ...webCamConfig, webCamDeviceId: e.target.value })}>
                                    {webCams.map((wc) => {
                                        return <option key={wc.deviceId} value={wc.deviceId}>{wc.label}</option>;
                                    })}
                                </select>
                                <label htmlFor="background"><b>Background</b></label>
                                <select id='background' value={webCamConfig.backgroundOption} onChange={(e) => setWebCamConfig({ ...webCamConfig, backgroundOption: parseInt(e.target.value) })}>
                                    {backgroundOptions.map((itm, itmidx) => {
                                        return <option key={itm} value={itmidx}>{itm}</option>;
                                    })}
                                </select>
                                <label htmlFor='mirror'><b>Mirror Enabled</b></label>
                                <input id='mirror' type='checkbox' checked={webCamConfig.mirrorEnabled} onChange={(e) => setWebCamConfig({ ...webCamConfig, mirrorEnabled: e.target.checked })} />
                                <label htmlFor="mic"><b>Microphone</b></label>
                                <select id='mic' value={webCamConfig.micDeviceId} onChange={(e) => setWebCamConfig({ ...webCamConfig, micDeviceId: e.target.value })}>
                                    {mics.map((wc) => {
                                        return <option key={wc.deviceId} value={wc.deviceId}>{wc.label}</option>;
                                    })}
                                </select>
            ...
            <img alt='bg' hidden={true} ref={operatorBackgrounRef} src={OperatorBackground} width={width} height={height} />
            <video hidden={!webCamConfig.showOriginal} ref={webCamRef} width={width} height={height} muted={true} className={webCamConfig.mirrorEnabled ? 'mirroredWebCamClass' : ''} >
            </video>
            <video hidden={true} ref={canvasRef} width={width} height={height} muted={true} className={webCamConfig.mirrorEnabled ? 'mirroredWebCamClass' : ''} >
            </video>
            <canvas hidden={true} ref={canvasForBackgroundRef} width={width} height={height} className={webCamConfig.mirrorEnabled ? 'mirroredWebCamClass' : ''} >
            </canvas>
            <video ref={resultVideoRef} autoPlay={true} width={width} height={height} muted={true} className={className + (webCamConfig.mirrorEnabled ? ' mirroredWebCamClass' : '')} >
            </video>
        </div>);
